...
def test_verb_count():
    assert len(VERBS) == 7
    assert VERBS == {
        "remember", "show", "filter", "count",
        "gather", "combine", "each",
    }


def test_connective_count():
    assert len(CONNECTIVES) == 9
    assert CONNECTIVES == {
...
def test_total_reserved_count_is_29():
    # v1c §47: 29 reserved words total.
    # 7 verbs + 9 connectives + 4 operators + 1 multi-word + 3 articles
    # + 3 v2 verbs + 2 v2 connectives = 29.
    assert len(ALL_RESERVED) == 29
...
def test_verb_signature_slot_shapes():
    # Sanity-check slot lists from inception §17, refined per v1b/v1d.
